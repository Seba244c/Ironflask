...
import dk.sebsa.ironflask.engine.utils.ListUtil;
...
public class HeightMapMesh {
	...
	public HeightMapMesh(float minY, float maxY, Texture heightMapFile, Texture texture, int textInc) throws Exception {
		this.minY = minY;
		this.maxY = maxY;
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            buf = stbi_load(heightMapFile.file.getAbsolutePath(), w, h, channels, 4);
            if (buf == null) {
        ...
        float[] posArr = ListUtil.listToArray(positions);
        int[] indicesArr = indices.stream().mapToInt(i -> i).toArray();
        float[] textCoordsArr = ListUtil.listToArray(textCoords);
    ...
    private float[] calcNormals(float[] posArr, int width, int height) {
        ...
        return ListUtil.listToArray(normals);
    }
